...
def load_protocol(data_dir: Path, slug: str) -> Protocol:
    path = data_dir / "protocols" / f"{slug}.yaml"
    if not path.exists():
...
def load_drug_depletions(data_dir: Path) -> list[DrugDepletion]:
    d = data_dir / "drug_depletions"
    if not d.exists():
        return []
    out = []
    for path in sorted(d.glob("*.yaml")):
        ...
        with path.open() as f:
            raw = yaml.safe_load(f)
        out.append(DrugDepletion(**raw))
    return out


def load_drug_depletion(data_dir: Path, slug: str) -> DrugDepletion:
    path = data_dir / "drug_depletions" / f"{slug}.yaml"
    if not path.exists():
        raise FileNotFoundError(f"drug_depletion not found: {slug}")
    with path.open() as f:
        raw = yaml.safe_load(f)
