...
**v0.29 (current)** — Path B turn 3: Plan editor complete + plan-check sidebar + Assess chat panel:
...
12. **Path B Next.js port — continue.** v0.29 finished the editor (10/10 tabs) + plan-check sidebar + chat panel. Remaining: tighten Patch type (silent typo on `lifestyle` vs `lifestyle_practices` was caught at runtime, not by tsc); auto-rehydrate chat history on page load; auto-scroll chat to latest; Claim picker for Education tab; dedup MultiSelect; session timeline on `/assess`; evidence-tier badges on suggestion items; Clients page; Resources Toolkit page; mechanism/symptom/claim detail pages; Mind Map Mermaid renderer; lifecycle transitions (submit/publish/revoke/supersede) currently CLI-only; Markdown/HTML export buttons on plan page.
