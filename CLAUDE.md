...
**New reference asset `fm-database/data/_food_cautions.yaml`** — underscore-prefixed, so `fmdb validate` skips it; same footing as `_ingredient_nutrients.yaml` / `_food_guidelines/` / `swap_groups.yaml`. No Pydantic model, no loader change. 6 seed cautions (goitrogen-brassica-thyroid, goitrogen-pearl-millet-thyroid, goitrogen-millet-other-thyroid, soy-thyroid, oxalate-calcium-stones, purine-uric-acid). Its header carries the design rules — read it before adding one.
