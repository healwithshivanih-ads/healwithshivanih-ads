...
SYSTEM_PROMPT = (
    ...
    "LIVER BIOTRANSFORMATION (DETOX) BURDEN — read it across sections:\n"
    "The intake doesn't have a 'detox' section, so the signals are scattered. "
    "When you see a CLUSTER of the following, surface ONE pattern + ONE "
    "`top_hypotheses` entry (not many) framed as impaired hepatic "
    "biotransformation / detox burden, and say which ROUTE it leans toward:\n"
    "  • LOAD (↑ Phase I demand): liver-metabolised meds (OCP/HRT, NSAIDs, "
    "PPIs, statins, psych meds, biologics) + regular alcohol + tobacco + "
    "frequent large/predatory fish + mould/solvent or declared toxic "
    "exposures.\n"
    "  • UPSTREAM (Phase I outpacing Phase II conjugation): chemical / "
    "fragrance sensitivity, strong hangovers, 'react to multiple meds + "
    "supplements', caffeine dependence ('need it' / headaches without), "
    "histamine signals, personal intolerance triggers.\n"
    "  • OESTROGEN CLEARANCE (Phase II, women): cyclical/cystic acne + "
    "period-linked or migraine headaches + severe period pain + "
    "perimenopause inventory.\n"
    "  • ELIMINATION (Phase III): < 1 bowel movement/day, hard/lumpy stool "
    "(Bristol 1-2), straining / incomplete evacuation, white tongue coating. "
    "This is the MOST actionable route — when elimination is backed up, say "
    "so explicitly and put 'support bowel regularity first' in "
    "verify_in_session, because recirculation makes the upstream phases look "
    "worse than they are.\n"
    "Confidence scales with how many routes light up; the classic 'backed-up "
    "detox' picture is LOAD + UPSTREAM + ELIMINATION together. Example label: "
    "'Detox / biotransformation burden — sluggish Phase III elimination + "
    "reactive-intermediate sensitivity' (confidence 0.6). Keep it a PATTERN "
    "TO EXPLORE, never a diagnosis or a confirmed 'stage' — phases can only be "
    "confirmed with functional labs. In verify_in_session you may note DUTCH "
    "oestrogen metabolites / organic acids / liver enzymes as clinician-"
    "ordered. NEVER recommend an aggressive detox or cleanse — support "
    "elimination + the basics (bowels, hydration, protein + sulfur/cruciferous "
    "foods, sleep) within coaching scope.\n\n"
