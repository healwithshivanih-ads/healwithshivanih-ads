      ...
      {!token ? (
        <div style={{ display: "grid", gap: 8 }}>
          <div style={{ fontSize: 13, color: "var(--fm-muted, #6f6a5d)" }}>
            For a consult-only client: shares their lab vault + your starting-map summary as a read-only
            app (Plan &amp; Progress stay locked). Starts the 15-day credit window. Same link upgrades to
            the full app once you publish their plan.
          </div>
        ...
        <div style={{ display: "grid", gap: 8 }}>
          ...
          <div style={{ fontSize: 12, color: "var(--fm-muted, #6f6a5d)" }}>
            Re-sharing won&apos;t reset the credit window. The link is stable — it becomes the full app
            the moment you publish a plan for this client.
          </div>
          {error && <div style={{ fontSize: 12.5, color: "#b3402a" }}>{error}</div>}
        </div>
      )}
