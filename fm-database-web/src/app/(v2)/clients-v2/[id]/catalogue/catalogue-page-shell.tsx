/**
 * Catalogue-tab shell — identity strip + 6-tab subnav (Catalogue active).
 * Mirrors communicate-page-shell.tsx; only the `activeNavId` and the
 * subnav highlight differ.
 */
...
export interface CataloguePageShellProps {
  clientId: string;
  children: React.ReactNode;
}

export async function CataloguePageShell({
  clientId,
  children,
}: CataloguePageShellProps) {
        ...
        {tabs.map((t) => (
          <Link
            ...
            style={{
              padding: "9px 16px",
              fontSize: 13,
              fontWeight: t.id === "catalogue" ? 700 : 500,
              color:
                t.id === "catalogue"
                  ? "var(--fm-text-primary)"
                  : "var(--fm-text-tertiary)",
              borderBottom: `2px solid ${t.id === "catalogue" ? "var(--fm-primary)" : "transparent"}`,
              textDecoration: "none",
              marginBottom: -1,
            }}
          >
