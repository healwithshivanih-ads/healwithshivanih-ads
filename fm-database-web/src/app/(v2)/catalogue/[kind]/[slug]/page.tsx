...
import { hasTracedFigure, tracedFigureSvg } from "@/lib/fmdb/exercise-figure-traced";
import type {
  CatalogueKind,
  Topic,
  Supplement,
...
function ExerciseDetail({ ex }: { ex: Exercise }) {
  const clientName = (ex.client_name || "").trim() || ex.display_name || ex.slug;
  const blocks = (ex.cautions ?? []).filter((c) => c.severity === "block");
  const softs = (ex.cautions ?? []).filter((c) => c.severity !== "block");
  // Only the true axes are worth showing — a wall of "No" teaches nothing.
  const flags: string[] = [
    ex.spinal_flexion ? "Spinal flexion" : "",
    ex.loads_spine ? "Loads the spine" : "",
    ex.loaded_spinal_rotation ? "Loaded rotation" : "",
    ex.overhead ? "Overhead" : "",
    ex.requires_floor_transfer ? "Needs floor transfer" : "",
  ].filter(Boolean);

  return (
    <div className="space-y-6">
      <Header
        title={ex.display_name ?? ex.slug}
        slug={ex.slug}
        tier={ex.evidence_tier}
        category={ex.modality?.replace(/_/g, " ")}
        extraBadges={
          <>
            {ex.intensity_tier && <Badge variant="outline">{ex.intensity_tier}</Badge>}
            {blocks.length > 0 && (
              <Badge variant="destructive">
                {blocks.length} hard block{blocks.length > 1 ? "s" : ""}
              </Badge>
            )}
          </>
        }
      ...
      <div className="flex gap-4 items-start">
        {/* The traced two-pose artwork where it exists; else the deterministic
            diagram computed from position + joint_stress + balance_demand (see
            exercise-figure.ts) so every entry still gets a figure. Both are
            generated by our own builders — the only interpolated free text is
            escaped inside them. */}
        <div
          className={`${hasTracedFigure(ex.slug ?? "") ? "w-44" : "w-20"} shrink-0 text-muted-foreground`}
          dangerouslySetInnerHTML={{
            __html:
              tracedFigureSvg(ex.slug ?? "", { title: clientName }) ??
              exerciseFigureSvg({
                position: ex.position,
                jointStress: ex.joint_stress,
                balanceDemand: ex.balance_demand,
                title: `${clientName} — ${ex.position?.replace(/_/g, " ") ?? "standing"}`,
              }),
          }}
        />
        <div className="flex-1 min-w-0 space-y-3">
          {/* The client-facing name is shown first and labelled, because the app
              previously rendered clinical display_names straight to clients. */}
          <div className="rounded-lg border bg-card p-3">
            <div className="text-[10px] uppercase tracking-wide text-muted-foreground font-semibold">
              Client sees
            </div>
            <div className="text-sm font-semibold mt-1">{clientName}</div>
          </div>
          {ex.summary && <p className="text-sm leading-relaxed">{ex.summary}</p>}
        </div>
      </div>

      <div className="grid gap-3 sm:grid-cols-4">
        {[
          { label: "Position", value: ex.position?.replace(/_/g, " ") },
          { label: "Impact", value: ex.impact },
          { label: "Balance demand", value: `${ex.balance_demand ?? 0} / 3` },
          ...
          f.value ? (
            <div key={f.label} className="rounded-lg border bg-card p-3">
              ...
              <div className="text-sm font-semibold mt-1">{f.value}</div>
            </div>
          ) : null,
        )}
      </div>
      ...
      {flags.length > 0 && (
        <Section title="Movement flags">
          <ChipList items={flags} />
        </Section>
      )}

      {blocks.length > 0 && (
        <Section title="Hard blocks — do not prescribe">
          <div className="space-y-2">
            {blocks.map((c, i) => (
              <div key={i} className="rounded-lg border border-destructive/40 bg-destructive/5 p-3">
                <div className="text-sm font-semibold">{c.condition}</div>
      ...
      {softs.length > 0 && (
        <Section title="Cautions — prescribe with the modification">
          <div className="space-y-2">
            {softs.map((c, i) => (
              <div key={i} className="rounded-lg border border-amber-500/40 bg-amber-500/5 p-3">
                <div className="text-sm font-semibold">{c.condition}</div>
                <p className="text-sm text-muted-foreground mt-1">{c.reason}</p>
                {c.modification && (
      ...
      {(ex.levels?.length ?? 0) > 0 && (
        <Section title="Levels — easiest first">
          <div className="space-y-2">
            {ex.levels!.map((lv, i) => (
              <div key={i} className="rounded-lg border bg-card p-3">
                <div className="flex items-center gap-2 flex-wrap">
                  <Badge variant="outline">{lv.level}</Badge>
                  <span className="text-sm font-semibold">{lv.prescription}</span>
                  {lv.support && (
                    <span className="text-xs text-muted-foreground">support: {lv.support}</span>
                  )}
                </div>
                {lv.note && <p className="text-sm text-muted-foreground mt-1">{lv.note}</p>}
              </div>
            ))}
          </div>
        </Section>
      ...
      {(ex.easier_variant || ex.harder_variant) && (
        <Section title="Progression ladder">
          <div className="flex items-center gap-2 flex-wrap text-sm">
            {ex.easier_variant && (
              <>
                <span className="text-muted-foreground">easier</span>
                <LinkedChipList items={[ex.easier_variant]} kind="exercises" />
              </>
            )}
            {ex.harder_variant && (
              <>
                <span className="text-muted-foreground">harder</span>
                <LinkedChipList items={[ex.harder_variant]} kind="exercises" />
              </>
            )}
          </div>
        </Section>
      ...
      {(ex.refer_out?.length ?? 0) > 0 && (
        <Section title="Refer out when">
          <PlainList items={ex.refer_out} />
        </Section>
      )}

      {(ex.builds?.length ?? 0) > 0 && (
        <Section title="Builds">
          <ChipList items={ex.builds} />
        </Section>
      )}
      {(ex.joint_stress?.length ?? 0) > 0 && (
        <Section title="Joint stress">
          <ChipList items={ex.joint_stress?.map((r) => r.replace(/_/g, " "))} />
        </Section>
      )}
      {(ex.equipment?.length ?? 0) > 0 && (
        <Section title="Equipment">
          <ChipList items={ex.equipment} />
        </Section>
      )}
      {(ex.linked_to_topics?.length ?? 0) > 0 && (
        <Section title="Conditions">
          <LinkedChipList items={ex.linked_to_topics} kind="topics" />
        </Section>
      )}
      {(ex.linked_to_symptoms?.length ?? 0) > 0 && (
        <Section title="Symptoms">
          <LinkedChipList items={ex.linked_to_symptoms} kind="symptoms" />
        </Section>
      )}

      {ex.why_it_works && (
        <Section title="Why it works">
          <p className="text-sm leading-relaxed whitespace-pre-line">{ex.why_it_works}</p>
        </Section>
      )}
      {ex.notes_for_coach && (
        <Section title="Notes for coach">
          <p className="text-sm leading-relaxed whitespace-pre-line">{ex.notes_for_coach}</p>
        </Section>
      )}

      <SourceCitations sources={ex.sources} />
    </div>
  );
}
...
export default async function CatalogueDetailPage({
  ...
  switch (kind) {
    ...
    case "exercises": {
      const ex = await loadOne<Exercise>("exercises", slug);
      if (!ex) notFound();
      body = <ExerciseDetail ex={ex} />;
      break;
    }
    case "protocols": {
