...
/** Human-friendly label per panel group (used by groupsForMarkers → the lab
 *  requisition + condition mapper). Unmapped groups fall back to the lowercased
 *  group name, so a new group never breaks — it just reads plainer. */
const FRIENDLY_GROUP: Record<string, string> = {
  "Standard Lab Panels": "standard panels",
  "Thyroid Function": "thyroid",
  "Blood Sugar & Insulin": "blood sugar",
  Inflammation: "inflammation",
  "Cardiovascular Risk": "heart risk",
  "Advanced Kidney & Metabolic": "liver & kidney",
  ...
  "Gut Health": "gut",
  // Bundle systems (a selected bundle / its component classifies to these,
  // not to the umbrella "Standard Lab Panels" — see groupsForMarkers).
  Kidney: "kidney",
  Liver: "liver",
  Lipids: "cholesterol",
...
/** Friendly group names that contain any of the given marker names — used by
 *  the lab requisition to summarise what was ordered. Bundle-aware: a marker
 *  that's a named bundle (KFT…) OR one of its components maps to the bundle's
 *  own SYSTEM (Kidney, Liver…) rather than the umbrella "Standard Lab Panels"
 *  group, so the slip summary reads by clinical system as it did before the
 *  base markers moved inside bundles. */
export function groupsForMarkers(markers: string[]): string[] {
  ...
  for (const panel of LAB_PANELS) {
    for (const l of panel.labs) {
      const isBundle = !!(l.components && l.components.length);
      const hit =
        want.has(l.name) || (isBundle && (l.components ?? []).some((c) => want.has(c)));
      if (!hit) continue;
      const groupKey = isBundle ? l.system ?? panel.group : panel.group;
      const friendly = FRIENDLY_GROUP[groupKey] ?? groupKey.toLowerCase();
