...
describe("buildActiveClientCountEvent", () => {
  ...
  const config = { maxActiveClients: 20, discoveryCallsPerWeek: 8 };

  it("active_clients = committed clients (maintenance excluded from cap)", () => {
    const ev = buildActiveClientCountEvent(breakdown, config, "2026-07-02T10:00:00.000Z");
    expect(ev.data).toMatchObject({
      active_clients: 12,
  ...
  it("event id is IST-minute-keyed (dedupes double fires, allows several/day)", () => {
    ...
    const ev = buildActiveClientCountEvent(breakdown, config, "2026-07-02T10:00:30.000Z");
    expect(ev.event_id).toBe("active_client_count:2026-07-02T15:30");
    const again = buildActiveClientCountEvent(breakdown, config, "2026-07-02T10:00:59.000Z");
    expect(again.event_id).toBe(ev.event_id);
    const later = buildActiveClientCountEvent(breakdown, config, "2026-07-02T10:01:10.000Z");
